            ...
            out Zip64ExtraField zip64Block)
        {
            ...
            try
            {
                ...
                using (BinaryReader reader = new BinaryReader(ms))
                {
                    ...
                    if (zip64Block._uncompressedSize < 0) throw new InvalidDataException("file uncompressed size too big");
                    if (zip64Block._compressedSize < 0) throw new InvalidDataException("file compressed size too big");
                    if (zip64Block._localHeaderOffset < 0) throw new InvalidDataException("file local header offset too big");
                    if (zip64Block._startDiskNumber < 0) throw new InvalidDataException("file start dis number too big");
